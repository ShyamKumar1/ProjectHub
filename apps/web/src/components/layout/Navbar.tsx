  ...
  useEffect(() => {
    if (user) {
      api.getStreaks().then((res) => {
        const total = (res.data || []).reduce((sum: number, s: any) => sum + (parseInt(s.total_active_days) || 0), 0);
        setStreakCount(total);
      }).catch(() => {});
    }
  }, [user]);
  ...
  return (
    <header className="h-16 border-b border-dark-300 flex items-center justify-between px-6 bg-dark-800/50 backdrop-blur-md">
      ...
      <div className="flex items-center gap-3">
        ...
        <Button variant="primary" size="sm" onClick={() => setCreateProjectOpen(true)} className="hidden sm:flex">
          <Plus size={16} />
          New Project
        </Button>
        ...
        <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-accent/10 border border-accent/20">
          ...
          <span className="text-sm font-medium text-accent">{streakCount}</span>
        </div>
